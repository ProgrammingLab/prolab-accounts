...
func (o *User) AuthorEntries(mods ...qm.QueryMod) entryQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"entries\".\"author_id\"=?", o.ID),
	)
...
func (o *User) InviterInvitations(mods ...qm.QueryMod) invitationQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"invitations\".\"inviter_id\"=?", o.ID),
	)
...
func (userL) LoadProfile(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.ProfileID) {
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.ProfileID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.ProfileID) {
	...
	query := NewQuery(qm.From(`profiles`), qm.WhereIn(`id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.ProfileID, foreign.ID) {
...
func (userL) LoadBlogs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`blogs`), qm.WhereIn(`user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadAuthorEntries allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadAuthorEntries(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`entries`), qm.WhereIn(`author_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.AuthorEntries = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &entryR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.AuthorID {
				local.R.AuthorEntries = append(local.R.AuthorEntries, foreign)
				if foreign.R == nil {
					foreign.R = &entryR{}
				}
...
func (userL) LoadInviterInvitations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`invitations`), qm.WhereIn(`inviter_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.InviterInvitations = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.InviterID {
				local.R.InviterInvitations = append(local.R.InviterInvitations, foreign)
				if foreign.R == nil {
					foreign.R = &invitationR{}
				}
...
func (o *User) SetProfile(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Profile) error {
	...
	queries.Assign(&o.ProfileID, related.ID)
	if o.R == nil {
		o.R = &userR{
			Profile: related,
		}
...
func (o *User) RemoveProfile(ctx context.Context, exec boil.ContextExecutor, related *Profile) error {
	...
	queries.SetScanner(&o.ProfileID, nil)
	if _, err = o.Update(ctx, exec, boil.Whitelist("profile_id")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}
	...
	for i, ri := range related.R.Users {
		if queries.Equal(o.ProfileID, ri.ProfileID) {
			continue
		}
...
// AddAuthorEntries adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.AuthorEntries.
...
func (o *User) AddAuthorEntries(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Entry) error {
	...
	if o.R == nil {
		o.R = &userR{
			AuthorEntries: related,
		}
	} else {
		o.R.AuthorEntries = append(o.R.AuthorEntries, related...)
	}
...
// AddInviterInvitations adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.InviterInvitations.
...
func (o *User) AddInviterInvitations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Invitation) error {
	...
	if o.R == nil {
		o.R = &userR{
			InviterInvitations: related,
		}
	} else {
		o.R.InviterInvitations = append(o.R.InviterInvitations, related...)
	}
