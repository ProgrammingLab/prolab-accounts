...
func testRoleToManyProfiles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadProfiles(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToManyAddOpProfiles(t *testing.T) {
	var err error
...
func testRoleToManySetOpProfiles(t *testing.T) {
	var err error
...
func testRoleToManyRemoveOpProfiles(t *testing.T) {
	var err error
...
var (
	roleDBTypes = map[string]string{`ID`: `bigint`, `Name`: `character varying`}
	_           = bytes.MinRead
)
