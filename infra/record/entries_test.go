...
func testEntryToOneBlogUsingBlog(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, entryDBTypes, true, entryColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Entry struct: %s", err)
	}
	...
	queries.Assign(&local.BlogID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
...
func testEntryToOneSetOpBlogUsingBlog(t *testing.T) {
	...
	for i, x := range []*Blog{&b, &c} {
		...
		if !queries.Equal(a.BlogID, x.ID) {
...
func testEntryToOneRemoveOpBlogUsingBlog(t *testing.T) {
	var err error
	...
	if err = a.SetBlog(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveBlog(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.Blog().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if !queries.IsValuerNil(a.BlogID) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.Entries) != 0 {
...
var (
	entryDBTypes = map[string]string{`ID`: `bigint`, `Title`: `character varying`, `Description`: `character varying`, `Content`: `character varying`, `Link`: `character varying`, `AuthorID`: `bigint`, `GUID`: `character varying`, `ImageFilename`: `character varying`, `BlogID`: `bigint`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_            = bytes.MinRead
)
